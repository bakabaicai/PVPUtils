...
public final class HelpCommand implements DotCommand {
    ...
    public void execute(String args) {
        ...
        StringBuilder commands = new StringBuilder(Config.isChinese
        ...
        if (IrcBridge.hasPrivilege("cape")) {
            commands.append(Config.isChinese
                    ? "，" + prefix + "irc cape <PNG文件完整路径>"
                    : ", " + prefix + "irc cape <full PNG path>");
        }
        if (IrcBridge.hasPrivilege("skin")) {
            commands.append(Config.isChinese
                    ? "，" + prefix + "irc skin <PNG文件完整路径>"
                    : ", " + prefix + "irc skin <full PNG path>");
        }
        ChatUtils.send(commands.toString());
    }
}
