...
public class RenderPage extends BasePage {
    public RenderPage() {
        ...
        modules.add(new SettingModule(UiText.t("攻击特效", "Attack Effects"), UiText.t("控制攻击时显示的粒子效果", "Control particles shown when attacking"), null)
                ...
                .addSub(UiText.t("闪电", "Lightning"), UiText.t("击杀目标时在死亡位置生成闪电", "Summon lightning at the target's death location when you kill it"),
                        new SettingToggle(() -> Config.attackEffectsLightning, v -> { Config.attackEffectsLightning = v; Config.save(); }))
                .addSubWhen(() -> Config.attackEffectsLightning, UiText.t("闪电数量", "Lightning Count"), UiText.t("击杀时生成的闪电数量", "Number of lightning bolts summoned on kill"),
        ...
        modules.add(new SettingModule(UiText.t("掉落物雷达", "Item Radar"), UiText.t("显示周围掉落物的名称与数量", "Show nearby dropped items and their counts"),
                new SettingToggle(() -> Config.droppedItemRadar, v -> { Config.droppedItemRadar = v; Config.save(); }))
                .addSub(UiText.t("样式", "Style"), UiText.t("切换 HUD 的渲染风格，可在 HUD 编辑器中调整位置", "Switch the HUD style; adjust position in the HUD editor"),
                        new SettingCycle(List.of(UiText.t("轻量", "Lite"), UiText.t("全新", "New"), UiText.t("模糊", "Blur")),
                                () -> Config.droppedItemRadarStyle.ordinal(),
                                i -> { Config.droppedItemRadarStyle = Config.HudStyle.values()[Math.max(0, Math.min(2, i))]; Config.save(); }))
                .addSub(UiText.t("渲染背景", "Background"), UiText.t("是否绘制 HUD 的背景面板", "Draw the HUD background panel"),
                        new SettingToggle(() -> Config.droppedItemRadarBackground, v -> { Config.droppedItemRadarBackground = v; Config.save(); })));
